...
public:
    ...
    void ProcessInput(std::vector<std::vector<ImVec2>>& strokes, 
        ...
        const bool is_touching = io.MouseDown[0] && io.MousePos.x >= 0.0f && io.MousePos.y >= 0.0f;
        const bool updated = is_touching;
        const ImVec2 touch_pos = io.MousePos;
#else
        if (fd < 0) {
        ...
        if (is_touching) {
            ...
            touch_pos = ImVec2(draw_x, draw_y);
        }
#endif

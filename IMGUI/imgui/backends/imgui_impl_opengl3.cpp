...
#ifdef STM32_DISPLAY_ROTATE_180
    // The physical STM32MP157F-DK2 panel in this installation is mounted
    // upside down, so only the production build rotates the framebuffer.
    float ortho_rotated[4][4];
    memcpy(ortho_rotated, ortho_projection, sizeof(ortho_projection));
    ortho_rotated[0][0] = -ortho_rotated[0][0];
    ortho_rotated[1][1] = -ortho_rotated[1][1];
    ortho_rotated[3][0] = -ortho_rotated[3][0];
    ortho_rotated[3][1] = -ortho_rotated[3][1];
#endif
    glUseProgram(bd->ShaderHandle);
    glUniform1i(bd->AttribLocationTex, 0);
#ifdef STM32_DISPLAY_ROTATE_180
    glUniformMatrix4fv(bd->AttribLocationProjMtx, 1, GL_FALSE, &ortho_rotated[0][0]);
#else
...
#ifdef STM32_DISPLAY_ROTATE_180
                // Rotate clip rect 180 degrees to match rotated projection
                ImVec2 clip_min_rot((float)fb_width - clip_max.x, (float)fb_height - clip_max.y);
                ImVec2 clip_max_rot((float)fb_width - clip_min.x, (float)fb_height - clip_min.y);
                GL_CALL(glScissor((int)clip_min_rot.x, (int)((float)fb_height - clip_max_rot.y), (int)(clip_max_rot.x - clip_min_rot.x), (int)(clip_max_rot.y - clip_min_rot.y)));
#else

...
public:
    ...
    bool Open() {
#ifdef DESKTOP_SIM
        // The WSL simulator has no RPMsg device.  Keep the same class API so
        // the application logic remains identical to the board build.
        return true;
#else
        if (fd >= 0) return true;
    ...
    void InjectCard(const std::string& uid) {
        simulated_card.Reset();
        simulated_card.uid = uid;
        simulated_card.card_type = "Simulated card";
        simulated_card.sak = "00";
        simulated_card.valid = !uid.empty();
    }
#endif
    ...
    RFIDData Poll() {
#ifdef DESKTOP_SIM
        RFIDData result = simulated_card;
        simulated_card.Reset();
        return result;
#else
        RFIDData result;
